...
Module.register("MMM-Domoticz-ext",{
    ...
    barometerIcon: "cloud-upload-alt",
    rainIcon: "cloud-rain",
    rooms: [],
    floors: [],
    dashboardRooms: [],
  ...
  loadingMessage: "Loading...",
    ...
    if (this.config.utilities.gaugeGasMinValue      == undefined) { this.config.utilities.gaugeGasMinValue      = this.defaults.utilities.gaugeGasMinValue;      }
...
getWeather: function(devices) {
  ...
  for (d in devices) {
    if (devices[d].type == "wind") {
      ...
      // 950 -> 999 hPa: 1 -> -0.5
      // 1000 -> 1050 hPa: -0.5 -> 0
      if      ( pressureInt >= 950   && pressureInt < 1000 ) { mappedPressure = pressureInt.map(this.baroMin, 999, 1, -0.5);  }
      else if ( pressureInt >= 1000  && pressureInt < 1050 ) { mappedPressure = pressureInt.map(1000, this.baroMax, -0.5, 0); }
      ...
      deviceRow.appendChild(cellWeatherBaro);
        ...
        // 0 -> 49 hPa: 1 -> -0.5
        // 50 -> 100 hPa: -0.5 -> 0
        if      ( humidityInt >= 0   && humidityInt < 49  ) { mappedHumidity = humidityInt.map(0, 49, 1, -0.5);  }
        else if ( humidityInt >= 50  && humidityInt < 100 ) { mappedHumidity = humidityInt.map(50, 100, -0.5, 0); }
